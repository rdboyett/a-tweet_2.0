...
def cache_result(default_size=settings.DEFAULT_SIZE):
    """
...
def invalidate_cache(user, size=None):
    ...
    """
    sizes = set(settings.AUTO_GENERATE_SIZES)
    if size is not None:
        sizes.add(size)
...
def get_default_avatar_url():
    ...
    if settings.DEFAULT_URL.startswith(('http://', 'https://')):
        return settings.DEFAULT_URL
    # We'll be nice and make sure there are no duplicated forward slashes
    ends = base_url.endswith('/')

    begins = settings.DEFAULT_URL.startswith('/')
    if ends and begins:
        base_url = base_url[:-1]
    elif not ends and not begins:
        return '%s/%s' % (base_url, settings.DEFAULT_URL)

    return '%s%s' % (base_url, settings.DEFAULT_URL)


def get_primary_avatar(user, size=settings.DEFAULT_SIZE):
    User = get_user_model()
